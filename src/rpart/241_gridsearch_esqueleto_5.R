...
ksemillas  <- c(100049, 100129, 100151, 235951, 297457) #reemplazar por las propias semillas
...
archivo_salida  <- "./labo/exp/HT2020/gridsearch_5.txt"
...
for( vmax_depth  in  c( 16, 18, 20, 22, 24, 26)  )
{
  for( vmin_bucket  in  c(3, 5, 7, 10, 20, 40, 100, 150, 200, 300, 400, 600)  )
  {
  for( vmin_split  in  c(3000, 2000, 1200, 1000, 800, 600, 400, 200, 100)  )
{

    if (vmin_bucket*2>vmin_split)
      next
    
    iteracion_n = iteracion_n + 1
    print(iteracion_n)
  ...
  print(c(vmax_depth, vmin_split, vmin_bucket, ganancia_promedio))
...
t2  <- Sys.time()
delta2_sec  <- as.numeric(  t2 - t0, units = "secs")  #calculo la diferencia de tiempos
print(delta2_sec) #imprimo
print(delta2_sec/60/60)