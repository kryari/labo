...
results  <- readxl::read_xlsx( "C:/Users/ARI/Desktop/ITBA/5.Mineria/labo/exp/ST7610/cluster_de_bajas_TareaCuatro_Ej3_base.xlsx")
campos_mean  <- c( "ctrx_quarter", "cpayroll_trx", "mcaja_ahorro", "mtarjeta_visa_consumo", "ctarjeta_visa_trx",
                     "mcuentas_saldo", "mrentabilidad_annual", "mprestamos_personales", "mactivos_margen", "mpayroll",
                     "Visa_mpagominimo", "Master_fechaalta", "cliente_edad", "chomebanking_trx", "Visa_msaldopesos",
                     "Visa_Fvencimiento", "mrentabilidad", "Visa_msaldototal", "Master_Fvencimiento", "mcuenta_corriente",
                     "Visa_mpagospesos", "Visa_fechaalta", "mcomisiones_mantenimiento", "Visa_mfinanciacion_limite",
                     ...
                     "mcomisiones", "Visa_cconsumos", "ccomisiones_otras", "Master_status", "mtransferencias_emitidas",
                     "mpagomiscuentas")
table_mean_cluster <- data.frame(matrix(NA, nrow=length(campos_mean), ncol=length(unique(results$cluster2))))

for (i in 1:length(campos_mean)){
table_mean_cluster[i,] <- results[  , mean(campos_mean[i]),  cluster2 ]  #media de la variable  ctrx_quarter
}
