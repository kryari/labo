...
#Defino la bayesiana
#Creo 2 tipos de parametros: numericos (cp) y enteros (otros)
#La "L" sirve para señalar que es un número entero
hs  <- makeParamSet(
          makeNumericParam("cp"       , lower= -1   , upper=    0.1),
          makeIntegerParam("minsplit" , lower=  1L  , upper= 8000L),  #la letra L al final significa ENTERO; el maximo podria haber sido 160.000
          makeIntegerParam("minbucket", lower=  1L  , upper= 2000L),
          makeIntegerParam("maxdepth" , lower=  3L  , upper=   20L),#el minimo podria haber sido 1 o 2
          forbidden = quote( minbucket > 0.5*minsplit ) )             # minbuket NO PUEDE ser mayor que la mitad de minsplit

ksemilla_azar  <- 100049   #cambiar por la primer semilla

#A mayor cantidad de parametros, mayor cantidad de iteraciones para alcanzar el maximo
...
#Usamos metodo de Cross Validation 
...
ctrl  <- makeMBOControl( save.on.disk.at.time= 600,  save.file.path= archivo_BO) #guarda cada 600 segundos
