...
AgregarVariables  <- function( dataset )
{
  ...
  #dataset[ , minversion_saldototal      :=  minversion_total/mv_msaldototal]
  #dataset[ , minversion_consumototal      := minversion_total / (mv_mconsumospesos+mv_mconsumosdolares) ]
  #dataset[ , minversion_payrroll      := minversion_total / mpayroll_total  ]
  #dataset[ , minversion_limitecompra      := minversion_total / mv_mlimitecompra ]  
  #dataset[ , minversion_cajaahorro      := minversion_total / mcaja_ahorro ]  
  
  #altas cargas de intereses (en terminos de saldos, consumos, etc.) pueden significar (o no) que el ciente quiera cambiar de banco, o que estÈ atado al actual
  dataset[ , mactivos_margen_saldototal      := mactivos_margen / mv_msaldototal ] 
  dataset[ , mactivos_margen_consumototal      := mactivos_margen / (mv_mconsumospesos+mv_mconsumosdolares) ]
  dataset[ , mactivos_margen_payrroll      := mactivos_margen / mpayroll_total  ] 
  dataset[ , mactivos_margen_limitecompra      := mactivos_margen / mv_mlimitecompra ]
  dataset[ , mactivos_margen_cajaahorro      := mactivos_margen / mcaja_ahorro ] 
  
  #altas cargas de comisiones (en terminos de saldos, consumos, etc.) pueden significar altos costos de quedarse para el cliente
  # dataset[ , mcomisiones_saldototal      := mcomisiones / mv_msaldototal ] 
  # dataset[ , mcomisiones_consumototal      := mcomisiones / (mv_mconsumospesos+mv_mconsumosdolares) ]
  # dataset[ , mcomisiones_payrroll      := mcomisiones / mpayroll_total  ] 
  # dataset[ , mcomisiones_limitecompra      := mcomisiones / mv_mlimitecompra ]
  # dataset[ , mcomisiones_cajaahorro      := mcomisiones / mcaja_ahorro ] 
  
  #busco normalizar la antiguedad. Personas jovenes con alto cociente antiguedad/edad indican fidelidad al banco
  #le resto 18 porque es el primer aÒo a partir del cual podrian tener cuenta
  #dataset[ , antiguedad_edad      := cliente_antiguedad*12 / (cliente_edad-18) ]
  
  #dummy por tener cierta actividad en dÛlares (puede entenderse como cliente m·s firme)
  # dataset[  ,  dolarizado := 0 ]
  # dataset[ minversion1_dolares>0 | mv_mpagosdolares>0 |
  ...
  #elevadas al cubo (para alejar m·s los valores entre sÌ)
  dataset[ , ctrx_quarter_cuad      := ctrx_quarter^2 ]
  dataset[ , cpayroll_trx_cuad      := cpayroll_trx^2 ] 
  dataset[ , mtarjeta_visa_consumo_cuad      := mtarjeta_visa_consumo^2 ] 
  dataset[ , mcaja_ahorro_cuad      := mcaja_ahorro^2 ] 
  dataset[ , mcuentas_saldo_cuad      := mcuentas_saldo^2 ] 
  dataset[ ,  cproductos_cuad      :=  cproductos^2 ]
  dataset[ ,  mpayroll_cuad      :=  cproductos^2 ]
  ...
  dataset[ foto_mes>202004,  foto_mes_new      :=  (foto_mes-202003)^2 ]
  ...
  dataset[ , mpasivos_margen_consumototal      := mpasivos_margen / (mv_mconsumospesos+mv_mconsumosdolares) ]
  dataset[ , mpasivos_margen_payrroll      := mpasivos_margen / mpayroll_total  ] 
  dataset[ , mpasivos_margen_limitecompra      := mpasivos_margen / mv_mlimitecompra ]
  dataset[ , mpasivos_margen_cajaahorro      := mpasivos_margen / mcaja_ahorro ] 
