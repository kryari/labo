...
modelo  <- lgb.train( data= dtrain,
                      param= list( objective=        "binary",
                                   ...
                                   max_bin=31,
                                   num_iterations=     159,
                                   learning_rate=0.0424048956620296,
                                   feature_fraction=   0.522356289051438,
                                   min_data_in_leaf= 1035,
                                   num_leaves=         563,
                                   lambda_l1=0.25276648650615,
                                   lambda_l2=9.25033225127036
                                     )
                    )
...
entrega  <- as.data.table( list( "numero_de_cliente"= dapply[  , numero_de_cliente],
                                 "Predicted"= as.integer(prediccion > 0.0148129291976869 ) )  ) #genero la salida

dir.create( "./labo/exp/",  showWarnings = FALSE ) 
dir.create( "./labo/exp/KA2512/", showWarnings = FALSE )
archivo_salida  <- "./labo/exp/KA2512/KA_512_001_TareaTres_Ej3_Prueba1.csv"
...
archivo_importancia  <- "./labo/exp/KA2512/512_importancia_001_TareaTres_Ej3_Prueba1.txt"
